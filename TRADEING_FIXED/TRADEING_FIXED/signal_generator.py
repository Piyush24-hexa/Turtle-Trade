...
def _run_technical_strategies(symbol, df, quote):
    ...
    # ── Strategy 1: Breakout ──
    if (ltp > prev20h * 1.001 and vol_ratio >= 1.5
    ...
    if bb_width < prev_bb_width * 0.7 and vol_ratio > 2.0:
...
def format_signal_telegram(sig: dict) -> str:
    """Format enriched signal as Telegram HTML."""
    is_buy = sig["signal_type"] in ("BUY", "BUY_CALL")
    arrow  = "🟢" if is_buy else "🔴"
    tier   = {"HIGH": "🔥", "MEDIUM": "📊", "LOW": "📌"}.get(sig.get("conviction","MEDIUM"), "📊")
    mode   = {"EQUITY":"📈","OPTIONS":"⚙️","CRYPTO":"🪙","FOREX":"💱"}.get(sig.get("mode","EQUITY"),"📈")
    ...
    lines = [
        f"{arrow} <b>{'BUY' if is_buy else 'SELL'} — {sig['symbol']}</b>",
        f"{tier} <i>{sig.get('conviction','?')} conviction | Score: {sig.get('overall_score',0):.0f}/100</i>",
        f"{mode} Mode: <b>{sig.get('mode','EQUITY')}</b>  |  Strategy: {sig.get('strategy','')}",
        "",
        "━━━━━━━━━━━━━━━━━━━━━━",
    ]

    if sig.get("technical_score"):
        lines.append(f"📐 Tech    <code>{bar(sig['technical_score'])}</code> {sig['technical_score']:.0f}%")
    if sig.get("ml_score"):
        lines.append(f"🧠 ML      <code>{bar(sig['ml_score'])}</code> {sig['ml_score']:.0f}%")
    if sig.get("sentiment_score"):
        lines.append(f"📰 News    <code>{bar(sig['sentiment_score'])}</code> {sig['sentiment_score']:.0f}%")
    if sig.get("pattern_score"):
        lines.append(f"🕯  Pattern <code>{bar(sig['pattern_score'])}</code> {sig['pattern_score']:.0f}%")
    if sig.get("fundamental_score"):
        lines.append(f"📊 Fund    <code>{bar(sig['fundamental_score'])}</code> {sig['fundamental_score']:.0f}%")
    ...
    lines += [
        ...
        f"🛡  SL:       <b>Rs.{sig['stop_loss']:,.2f}</b> (-{sig['risk_pct']:.2f}%)",
        f"⚖  R:R = 1:{sig['risk_reward']:.1f}  |  Confidence: {sig['confidence']}%",
        "",
        f"📦 {sig.get('quantity',0)} shares | Rs.{sig.get('investment',0):,.0f} | Risk Rs.{sig.get('risk_amount',0):,.0f}",
        "",
        "━━━━━━━━━━━━━━━━━━━━━━",
    ...
    if sig.get("pattern"):
        lines.append(f"🕯  Pattern: <b>{sig['pattern'].replace('_',' ')}</b>")
    if sig.get("rf_label"):
        lines.append(f"🤖 RF: <b>{sig['rf_label']}</b> ({sig.get('rf_confidence',0):.0%}) | LSTM: <b>{sig.get('lstm_direction','?')}</b>")
    if sig.get("news_headline"):
        tag = {"POSITIVE":"🟢 Bullish","NEGATIVE":"🔴 Bearish","NEUTRAL":"⚪ Neutral"}.get(sig.get("sentiment",""), "")
        lines.append(f"📰 {tag}: <i>\"{sig['news_headline']}\"</i>")
    if sig.get("market_state"):
        lines.append(f"🌐 Market: {sig['market_state']} | VIX: {sig.get('vix',0):.1f}")

    lines += [
        "",
        f"🕐 {sig.get('timestamp', datetime.now().strftime('%H:%M'))}  |  " +
        ("⚠️ PAPER TRADE" if sig.get("paper_trade", True) else "🔴 LIVE TRADE"),
    ]
    return "\n".join(lines)


# ─────────────────────────────────────────────────
# POSITION SIZING (standalone helper)
# ─────────────────────────────────────────────────
