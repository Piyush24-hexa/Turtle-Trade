...
def save_signal(signal: dict) -> int:
    """Save a trading signal to DB. Returns row ID."""
    try:
        conn = sqlite3.connect(config.DB_PATH)
        cur = conn.execute(
            """INSERT INTO signals
               (timestamp, symbol, signal_type, strategy, entry_price, target_price,
                stop_loss, confidence, risk_reward, position_size, investment, risk_amount, reason)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                signal.get("timestamp", datetime.now().isoformat()),
                signal["symbol"], signal["signal_type"], signal.get("strategy"),
                signal["entry_price"], signal["target_price"], signal["stop_loss"],
                signal.get("confidence", 0), signal.get("risk_reward", 0),
                signal.get("position_size", 0), signal.get("investment", 0),
                signal.get("risk_amount", 0), signal.get("reason", ""),
            )
        )
        conn.commit()
...
def get_today_signals() -> list:
    """Fetch all signals generated today."""
    try:
        conn = sqlite3.connect(config.DB_PATH)
